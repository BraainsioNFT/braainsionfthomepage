...
type CharacterProps = {
  ...
  animatedBackgroundContainerRef: React.RefObject<HTMLDivElement>;
  animatedBackgroundCharacterImagesRefs: React.RefObject<Element>[];
...
export class Character extends Component<CharacterProps> {
  ...
  move() {
    if (this.xVel > this.characterMaxSpeed * this.smallestCanvasDimension()) {
      this.xVel = this.characterMaxSpeed * this.smallestCanvasDimension();
    }
    if (this.yVel > this.characterMaxSpeed * this.smallestCanvasDimension()) {
      this.yVel = this.characterMaxSpeed * this.smallestCanvasDimension();
    }
    if (this.xVel < -this.characterMaxSpeed * this.smallestCanvasDimension()) {
      this.xVel = -this.characterMaxSpeed * this.smallestCanvasDimension();
    }
    ...
    this.x +=
      this.xVel +
      (this.xVel > 0
        ? this.characterAcceleration
        : -this.characterAcceleration) *
        this.smallestCanvasDimension();
    this.y +=
      this.yVel +
      (this.yVel > 0
        ? this.characterAcceleration
  ...
  draw(context: CanvasRenderingContext2D) {
    ...
    context.rotate(this.degToRad(this.direction + 90));
    context.drawImage(
      this.image,
      -this.imageWidth / 2,
      -this.imageHeight / 2,
  ...
  randomCharacterImage() {
    return this.props.animatedBackgroundCharacterImagesRefs[
      Math.floor(
        Math.random() * this.props.animatedBackgroundCharacterImagesRefs.length
      )
    ];
  }
  spawnCharacters(numCharacters: number) {
    [...new Array(numCharacters)].map(() =>
      this.characters.push(new Character(this.props))
    );
  }
