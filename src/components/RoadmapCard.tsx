...
type RoadmapCardProps = { id: number, title: string, desc: string, dispDesc: JSX.Element, setDisp: Dispatch<SetStateAction<number>>, isDisplayed?: boolean, isHovered?: boolean, icon?: string, dateComplete: string }
	...
	return (
		...
		>
			<div className={`w-[90%] ${props.isDisplayed ? "h-full scale-50 xs:scale-75 md:scale-110" : "h-64"} relative transition overflow-hidden shadow-drop bg-green-700 bg-opacity-75 text-opacity-100 text-white rounded-md ${props.isHovered && !props.isDisplayed ? "blur" : ""}`}>
				...
				<div className={"h-16"}>
					<h1 className={`text-3xl text-center ${props.isDisplayed ? "pt-5" : "pt-2"}`}>{props.title ?? "Subsequent Stage"}</h1>
					<h3 className={`text-1xl text-center pb-2`}>{props.dateComplete ?? "TBD"}</h3>
				</div>
