...
const RoadmapCard = (props: RoadmapCardProps) => {
	return (
		...
		>
			<div className={`w-[90%] ${props.isDisplayed? "h-full" : "h-64"} relative transition overflow-hidden shadow-drop bg-emerald-300 rounded-md ${props.isHovered && !props.isDisplayed? "blur" : ""}`}>
				<button onClick={() => {
					props.setDisp(0);
					...
					window.scrollBy({ left: 0, top: -1});
				}} className={`${props.isDisplayed? "absolute top-5 right-5 rounded-full border-2 border-green-900 text-green-900 hover:bg-green-900 hover:text-white h-10 aspect-square" : "hidden"}`}>
					X
				</button>
				...
				<div className={`flex ${props.isDisplayed? "h-96" : "h-48"}`}>
					...
					<div className={props.isDisplayed? "w-full relative px-2" : "w-1/2 relative px-2"}>
						<p className={`absolute top-0 bottom-0 my-auto h-fit ${props.isDisplayed? "px-20" : ""}`}>
							{
								props.isDisplayed?
			...
			<div className={!props.isDisplayed && props.isHovered? "left-0 right-0 bottom-0 top-0 m-auto h-64 absolute flex items-center justify-center" : "hidden"}>
				<button className="rounded-lg shadow-md border-green-900 text-green-900 border-2 text-3xl hover:bg-green-900 hover:text-white transition p-2"
				onClick={() => {
